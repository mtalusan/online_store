...
<?php
	...
	// If this page was passed an item_ID, add
	// it to the wishlist
	if(@$item_ID != NULL)
	{
		$add_rs = $pdo->prepare("INSERT INTO Wishlist VALUES(:item_ID, :Customer_ID);");
	...
	if($item_ID_list)
	{
		...
		foreach($item_ID_list as $item)
		{
			...
			$item_name = $item_name_rs->fetchAll(PDO::FETCH_ASSOC);
			...
			echo "<input type=\"hidden\" name=\"item_ID\" value=\"" . $item["Product_ID"] . "\"/>";
